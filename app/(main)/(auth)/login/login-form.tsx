...
function ResendVerificationButton({
  ...
  const label =
    state.kind === "sending"
      ? tSignup("resending")
      : secondsLeft > 0
        ? tSignup("resendIn", { seconds: secondsLeft })
        : t("resendVerification");

  const hint =
    state.kind === "rateLimited" &&
    (state.reason === "email_daily" || state.reason === "global_daily")
      ? tSignup("resendDaily", { hours: Math.ceil(secondsLeft / 3600) })
      : state.kind === "rateLimited"
        ? tSignup("resendTooMany", { seconds: secondsLeft })
