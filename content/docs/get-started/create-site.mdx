...
1. Log into the [yCAPTCHA dashboard](https://ycaptcha.xyspg.moe/dashboard).
...
| Prompt           | The instruction shown to users (e.g., `Select all images with\|parks`).                     |
...
Prompts support a `|` separator to split the instruction from the keyword:

- `Select all images with|parks` — renders "Select all images with" on one line and **parks** as a large bold keyword.
- Without `|`, the entire prompt is used as the keyword with a default instruction.
